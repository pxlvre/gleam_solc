...
"
  ...
  dict.each(sources, fn(filename, source) {
    io.println("- " <> filename <> " (" <> int.to_string(string.length(source)) <> " chars)")
  })
}
...
"
  ...
  let optimizer = types.OptimizerSettings(
    enabled: True,
    runs: 1000000  // Optimize for many function calls
  )
  
  let settings = types.CompilationSettings(
    output_selection: output_selection,
    optimizer: Some(optimizer),
    evm_version: Some("london"), // Use London hard fork features
    libraries: None,
    remappings: None
  )
  ...
  let input = types.CompilationInput(
    language: "Solidity",
    sources: dict.from_list([#("OptimizedContract.sol", types.Source(content: source))]),
    settings: settings
  )
  ...
  io.println("Output selections: " <> int.to_string(list.length([
    "abi", "evm.bytecode", "evm.deployedBytecode", "evm.gasEstimates",
    "evm.methodIdentifiers", "metadata", "storageLayout", "devdoc", "userdoc"
  ])))
}
...
"
  ...
  let compilation_error = types.CompilationError(
    severity: "error",
    message: "Expected ';' but got '}'",
    formatted_message: Some("ParserError: Expected ';' but got '}' at InvalidSyntax.sol:3:21"),
  ...
  // 2. Version errors
  let version_error = types.VersionNotFound("Version 0.8.999 not found in releases")
  io.println("2. Version Error: " <> get_error_message(version_error))
  
  // 3. Download errors
  let download_error = types.DownloadError("Failed to download: Network timeout")
...
pub fn output_analysis_example() {
  ...
  let mock_bytecode = types.Bytecode(
    ...
    source_map: Some("1:2:0:-:0;;;8:1:-1;5:2;;;30:1;27;20:12;5:2;1:2:0;;;;;;;")
  )
  
  let mock_gas = types.GasEstimates(
  ...
  io.println("Bytecode size: " <> int.to_string(string.length(mock_contract.evm.bytecode.object)))
