...
import solc/types.{
  type CompilationInput, type CompilationOutput, type SolcError,
  type SolcWrapper,
}
import solc/wrapper
...
) -> Promise(Result(SolcWrapper, SolcError)) {
...
fn create_multiple_input(sources: dict.Dict(String, String)) -> CompilationInput {
  let gleam_sources =
    dict.map_values(sources, fn(_, content) { types.Source(content: content) })
...
fn encode_sources(sources: dict.Dict(String, types.Source)) -> json.Json {
  ...
  let json_pairs =
    list.map(source_pairs, fn(pair) {
      let #(name, source) = pair
      #(name, json.object([#("content", json.string(source.content))]))
    })
...
fn encode_output_selection(selection: types.OutputSelection) -> json.Json {
  ...
  let json_pairs =
    list.map(selection_pairs, fn(pair) {
      let #(file, contracts) = pair
      let contract_pairs = dict.to_list(contracts)
      let contract_json_pairs =
        list.map(contract_pairs, fn(contract_pair) {
