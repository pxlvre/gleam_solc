...
pub fn download(path: String, version: Option(SolcVersion)) -> Promise(Result(SolcVersion, SolcError)) {
  ...
  case dict.get(release_info.releases, target_version) {
    Ok(filename) -> {
      let url = "https://binaries.soliditylang.org/emscripten-wasm32/" <> filename
    ...
    Error(_) -> {
      promise.resolve(Error(types.VersionNotFound("Version " <> target_version <> " not found")))
    }
  }
}
...
pub fn fetch_releases() -> Promise(Result(ReleaseInfo, SolcError)) {
  ...
  case response {
    Ok(data) -> {
      ...
      |> result.map_error(fn(msg) { types.DownloadError("Failed to parse releases: " <> msg) })
    }
    Error(msg) -> Error(types.DownloadError("Failed to fetch releases: " <> msg))
  }
}
...
fn release_info_decoder() -> decode.Decoder(ReleaseInfo) {
  use releases <- decode.field("releases", decode.dict(decode.string, decode.string))
...
pub fn get_filename_for_version(version: SolcVersion) -> Promise(Result(SolcVersion, SolcError)) {
  ...
  case release_info {
    Ok(info) -> {
      case dict.get(info.releases, version) {
        ...
        Error(_) -> Error(types.VersionNotFound("Version " <> version <> " not found"))
      }
    }
    Error(err) -> Error(err)
  }
