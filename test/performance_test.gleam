...
pub fn type_construction_benchmark() {
  ...
  io.println("Type construction (1000 instances): " <> int.to_string(duration) <> "ms")
}
...
"
  ...
  let _input = types.CompilationInput(
    language: "Solidity",
    sources: sources,
    settings: settings
  )
  ...
  should.be_true(duration >= 0)
  should.be_true(string.length(large_source) > 10000)
  io.println("Large input creation (" <> int.to_string(string.length(large_source)) <> " chars): " <> int.to_string(duration) <> "ms")
}
...
"
  ...
  io.println("Multi-file input (50 contracts): " <> int.to_string(duration) <> "ms")
}
...
pub fn abi_construction_benchmark() {
  ...
  let functions = list.range(1, 100)
    |> list.map(fn(i) {
      types.ABIFunction(
  ...
  io.println("Complex ABI construction (120 items): " <> int.to_string(duration) <> "ms")
}
...
pub fn error_handling_benchmark() {
  ...
  // Process errors
  let _processed = list.map(errors, fn(error) {
    case error {
  ...
  io.println("Error handling (1000 errors): " <> int.to_string(duration) <> "ms")
}
...
pub fn json_encoding_benchmark() {
  ...
  let large_dict = list.range(1, 500)
    |> list.map(fn(i) {
  ...
  let _processed = dict.map_values(large_dict, fn(k, v) {
    k <> ":" <> v
  })
  ...
  io.println("Dictionary processing (500 entries): " <> int.to_string(duration) <> "ms")
}
...
pub fn run_all_benchmarks() {
  ...
  io.println("Note: These are structural benchmarks showing the library can handle large inputs efficiently.")
  io.println("Real performance would depend on the actual Solidity compilation time.")
}