...
pub fn error_handling_test() {
  ...
  case compile_error {
    types.CompilationFailed(msg) ->
      should.equal(msg, "Syntax error in contract")
  }
...
pub fn bytecode_structure_test() {
  ...
  let bytecode =
    types.Bytecode(
      ...
      link_references: dict.from_list([#("Math.sol", [link_ref])]),
...
pub fn evm_structure_test() {
  ...
  let gas_estimates =
    types.GasEstimates(
      ...
      external: dict.from_list([#("transfer(address,uint256)", "24755")]),
    )
